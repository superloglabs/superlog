...
// Loads dotenv first so OTEL_* env vars set in .env / .env.superlog /
// .env.local are visible before the SDK reads process.env.
// Uses HTTP/protobuf exporters (no gRPC).
import "dotenv/config";
import { config as loadDotenv } from "dotenv";
const portlessPort = process.env.PORTLESS_URL ? process.env.PORT : undefined;
// .env.superlog holds the OTLP endpoint + ingest auth header; it's the
// dedicated home for OTel config so it doesn't collide with app .env.
loadDotenv({ path: ".env.superlog" });
// .env.local wins so per-developer dogfood / worktree overrides take effect.
...
if (telemetryEnabled) {
  const serviceName = process.env.OTEL_SERVICE_NAME ?? "@superlog/api";
