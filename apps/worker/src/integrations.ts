  ...
  for (const integration of integrations) {
    for (const op of integration.definition.operations) {
      tools.push({
        ...
        input_schema: sanitizeManagedAgentsInputSchema(op.input_schema),
      });
    }
  }
  return tools;
...
// Anthropic's Managed Agents API rejects unknown keys in `input_schema` with
// "Extra inputs are not permitted" (Pydantic extra=forbid). Integration authors
// write standard JSON Schema with `additionalProperties`, `format`, `minLength`,
// etc.; strip everything outside the documented allowlist at this boundary so
// the integration registry stays readable.
...
function sanitizeManagedAgentsInputSchema(schema: unknown): {
  type: "object";
...
export async function dispatchPendingIntegrationToolCalls(
  client: Anthropic,
  ...
  const opsByName = new Map<
    string,
    { integration: ResolvedIntegration; op: IntegrationOperation }
  >();
  for (const integration of integrations) {
    for (const op of integration.definition.operations) {
      opsByName.set(op.name, { integration, op });
    }
  }

  const events: Array<
    | { kind: "use"; id: string; name: string; input: Record<string, unknown> }
    | { kind: "result"; useId: string }
  > = [];
  for await (const event of client.beta.sessions.events.list(ctx.sessionId)) {
    if (event.type === "agent.custom_tool_use") {
      ...
      events.push({ kind: "result", useId: event.custom_tool_use_id });
    }
  }
  const responded = new Set(
    events
      .filter((e): e is { kind: "result"; useId: string } => e.kind === "result")
      .map((e) => e.useId),
  );
  const useCountByName = new Map<string, number>();
  for (const e of events) {
    if (e.kind === "use") useCountByName.set(e.name, (useCountByName.get(e.name) ?? 0) + 1);
  }

  let dispatched = 0;
  for (const e of events) {
    if (e.kind !== "use" || responded.has(e.id)) continue;
    const entry = opsByName.get(e.name);
    // Not an integration tool — leave it for whichever collector owns that tool
    // (e.g. submit_agent_run_result is acked by collectManagedAgentRun).
    // We deliberately don't error-ack here because that would race the owning
    // collector and can corrupt its terminal handoff.
    if (!entry) continue;
    const { integration, op } = entry;

    if (
      typeof op.rate_limit_per_session === "number" &&
      (useCountByName.get(op.name) ?? 0) > op.rate_limit_per_session
    ) {
      await sendToolError(
        client,
        ctx.sessionId,
        e.id,
        `Rate limit exceeded: ${op.name} is capped at ${op.rate_limit_per_session} calls per agent run.`,
      );
      dispatched++;
      continue;
    }

    const validation = validateInput(e.input, op.input_schema);
    if (validation) {
      await sendToolError(client, ctx.sessionId, e.id, `Invalid input: ${validation}`);
      dispatched++;
      continue;
    }

    try {
      const result = await executeOperation(integration, op, e.input, {
        incident_id: ctx.incidentId,
        session_id: ctx.sessionId,
      });
      const filtered = filterResponse(result, op.response_filter);
      await client.beta.sessions.events.send(ctx.sessionId, {
        events: [
          {
      ...
      log.error({ err, op: op.name, sessionId: ctx.sessionId }, "integration operation failed");
      await sendToolError(client, ctx.sessionId, e.id, msg);
    }
    dispatched++;
  }
  return dispatched;
...
function validateInput(input: unknown, schema: Record<string, unknown>): string | null {
...
function filterResponse(value: unknown, allowed: string[] | undefined): unknown {
