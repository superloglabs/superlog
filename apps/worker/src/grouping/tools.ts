...
export const GROUPING_SYSTEM_PROMPT = [
  ...
  "For an alert issue, inspect whether its observed population is already accounted for by linked issue event counts in one candidate incident during the same window. If it is, join the alert to that incident; the aggregate alert is not a new root cause.",
