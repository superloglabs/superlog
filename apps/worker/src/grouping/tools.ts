...
export const GROUPING_SYSTEM_PROMPT = [
  ...
  "Reconstruct causal order across components. A callee fault, the caller's resulting failure, and an error handler that then fails while reporting that failure are one causal chain, not three independent root causes.",
  "Matching first-seen time, last-seen time, and event count across repeated symptoms is strong evidence that the same attempts produced every link in a causal chain, especially when service, environment, release, repository, or request identifiers also match.",
  "For an alert issue, inspect whether its observed population is already accounted for by linked issue event counts in one candidate incident during the same window. Candidate eventCount and firstSeen are included only when scoped safely to that incident window. If the population is accounted for, join the alert to that incident; the aggregate alert is not a new root cause.",
