...
export const GROUPING_SYSTEM_PROMPT = [
  ...
  "A shared code location is NOT a shared root cause. Different exception types (e.g. a TypeError and a ReferenceError, or a SyntaxError and a timeout Error) that surface in the same endpoint, handler, or file are SEPARATE incidents by default — they are distinct bugs that merely live in the same place. Join them only when one causally produces the other, or they are observed within the same trace tree. Distinct exception types with distinct top frames and no shared trace id are standalone.",
